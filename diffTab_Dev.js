...
var pathWriteResult = "./DiffApps/Results/loadScript/";
...
//	diffTab: function () {
		fs.readdir(pathReadTabOrder, function (err, files) {
			...
			for (i = 0; i < files.length; i++) {
				...
				AppNames.indexOf(AppName) === -1 ? AppNames.push(AppName) : console.log("Item exists 1");		
			}
			...
			for (j = 0; j < AppNames.length; j++) {
				...
				for (k = 0; k < files.length; k++) {
					...
					TabNames.indexOf(TabName) === -1 ? TabNames.push(TabName) : console.log("Item exists 2");
					}
				}
			...
			for (i = 0; i < AppTabNames.length; i++) {
				for (j = 1; j < AppTabNames[i].length; j++) {
					...
					var originalExists = fs.existsSync(pathReadTabOrder + AppTabNames[i][0] + '.Original!' + AppTabNames[i][j] + '.txt');
					var contenderExists = fs.existsSync(pathReadTabOrder + AppTabNames[i][0] + '.Contender!' + AppTabNames[i][j] + '.txt');
					var UnderscoreIsAt = AppTabNames[i][j].indexOf('_');
					var TabNameWithoutTabNumber = AppTabNames[i][j].substring(UnderscoreIsAt +1);
					var original2Exists = fs.existsSync(pathReadNoTabOrder + AppTabNames[i][0] + '.Original!' + TabNameWithoutTabNumber + '.txt');
					var contender2Exists = fs.existsSync(pathReadNoTabOrder + AppTabNames[i][0] + '.Contender!' + TabNameWithoutTabNumber + '.txt');
					
					if(originalExists == true && contenderExists == true) {
//Both files found with same TabNr
						var original = fs.readFileSync(pathReadTabOrder + AppTabNames[i][0] + '.Original!' + AppTabNames[i][j] + '.txt', 'utf8');
						var contender = fs.readFileSync(pathReadTabOrder + AppTabNames[i][0] + '.Contender!' + AppTabNames[i][j] + '.txt', 'utf8');
						var diff = jsdiff.diffTrimmedLines(original, contender);
						console.log('Create file: ' + AppTabNames[i][0] + '!' +  TabNameWithoutTabNumber);
						...
						diff.forEach(function(part){
							...
							fs.appendFile(pathWriteResultFullMatchFound + AppTabNames[i][0] + '!' +  TabNameWithoutTabNumber + '.txt', [status]+ part.count + '|'  + part.value, function(err) {
								console.log(err);
							});
...
//If no file found store in separate array for second try without tab number in case order of tab has changed. 				
						var original2 = fs.readFileSync(pathReadNoTabOrder + AppTabNames[i][0] + '.Original!' + TabNameWithoutTabNumber + '.txt', 'utf8');
						var contender2 = fs.readFileSync(pathReadNoTabOrder + AppTabNames[i][0] + '.Contender!' + TabNameWithoutTabNumber + '.txt', 'utf8');
						...
						diff2.forEach(function(part){
							var status = part.added ? 'Add@' : part.removed ? 'Rem@' : '===@';
							fs.appendFile(pathWriteResultPartMatchFound + AppTabNames[i][0] + '!' +  TabNameWithoutTabNumber + '.txt', [status]+ part.count + '|'  + part.value, function(err) {
								console.log(err);
							});
						});
					} else {
//One of the files are not found
						...
						if (originalExists == true) {
							fs.copyFile(pathReadTabOrder + AppTabNames[i][0] + '.Original!' + AppTabNames[i][j] + '.txt', pathWriteResultNoMatchFound + AppTabNames[i][0] + '.Original!' + AppTabNames[i][j] + '.txt', (err) => {
								if (err) throw err;
							});

						} else {

							fs.copyFile(pathReadTabOrder + AppTabNames[i][0] + '.Contender!' + AppTabNames[i][j] + '.txt', pathWriteResultNoMatchFound + AppTabNames[i][0] + '.Contender!' + AppTabNames[i][j] + '.txt', (err) => {
								if (err) throw err;
							});
